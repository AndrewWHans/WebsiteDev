/*
  # Fix ticket verification functionality

  1. Changes
    - Add redemption functionality to verify_ticket function
    - Add proper error handling for invalid tickets
    - Add verification status tracking
    - Add driver role check
    - Return complete ticket information

  2. Security
    - Maintain existing RLS policies
    - Add role-based verification checks
*/
...
BEGIN
  -- Check if verifier is a driver (if verifier ID is provided)
  IF p_verifier_id IS NOT NULL THEN
    SELECT role INTO v_verifier_role
    FROM profiles
    WHERE id = p_verifier_id;

    IF v_verifier_role != 'Driver' THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Only drivers can verify tickets'
      );
    END IF;
  END IF;
