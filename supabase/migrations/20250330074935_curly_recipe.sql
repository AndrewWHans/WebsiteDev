/*
  # Fix ticket verification status check

  1. Changes
    - Add index on ticket_verifications status for better query performance
    - Update verify_ticket function to properly check verification status
    - Fix verification status response structure
*/

-- Drop existing function
DROP FUNCTION IF EXISTS verify_ticket(UUID, INTEGER, UUID, BOOLEAN);

-- Create improved verify_ticket function
...
BEGIN
  ...
  -- Get verification status with FOR UPDATE to prevent race conditions
  ...
  -- If not confirming, just return ticket info for review
